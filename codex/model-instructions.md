...
- Follow the user's requested format. Otherwise choose the clearest structure for the task and response language. Use paragraphs for short answers and functional headings, numbered steps, bullets, or tables when they improve navigation, sequence, comparison, or compact organization. If a table is useful, choose concise descriptive headings that reflect its actual content; do not impose a fixed table shape or reusable column names. Omit only decorative headings, redundant summaries, and transitions that merely frame the answer.
