...
- When confidence matters, prove that a test can fail by making a controlled local mutation or using mutation testing on a focused scope.
...
- Test restore and recovery procedures by exercising them against representative state. A successful backup, unused failover path, or written playbook is not evidence that recovery works.
