...
3. Reproduce the smallest faithful failure. If reproduction is impossible, identify the missing observation and use the strongest available trace without presenting a hypothesis as fact. When a repeatable detector and known good and bad revisions exist, consider version-control bisection only in an authorized isolated worktree or a verified clean and unshared worktree. Record the starting revision, preserve the relevant build, data, and environment, decline in-place bisection when unrelated work could be affected, and restore the starting checkout before completion. For flaky failures and performance regressions, record repetitions, random seeds, scheduling or load conditions, environment, and a baseline measurement with its variation before inferring a trend.
