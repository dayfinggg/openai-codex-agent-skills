...
5. Add or update focused tests when the repository has an appropriate pattern. Validate boundary cases, failure behavior, security-sensitive inputs, resource handling, and concurrency only where relevant. For retained code with a material performance, scaling, shared-state, asynchronous, retry, resource-lifecycle, dependency, or version-sensitive API risk, apply the corresponding [performance](../production-code-quality/references/performance.md), [concurrency and resilience](../production-code-quality/references/concurrency-resilience.md), or [API contract](../production-code-quality/references/api-contracts.md) procedure and record its evidence in the map.
6. Run the narrowest useful formatter, static analysis, type check, test, build, and user-visible verification available, then broaden according to the mapped risk. Record commands, artifacts, and any constraints in the map. Do not claim a quality attribute improved unless the check observes that attribute under a relevant condition.
