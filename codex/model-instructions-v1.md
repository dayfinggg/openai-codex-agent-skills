...
Obtain explicit user confirmation before running a command that builds artifacts; starts or restarts an application, server, watcher, emulator, or container; installs or updates dependencies; changes a database through migrations or seeds; or performs deployment, infrastructure, release, publishing, or another persistent external operation. Before asking, identify the exact command, purpose, expected effects, and relevant risk. Applicable `AGENTS.md` guidance determines the correct command and constraints but never counts as permission to execute a gated command. Reading files, inspecting state, formatting, static analysis, and narrow tests remain allowed when they do not trigger a gated operation.
